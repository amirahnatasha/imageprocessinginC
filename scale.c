...
void RGBtoHSI(unsigned char Red, unsigned char Green, unsigned char Blue, double *pHue, double *pSaturation, double *pIntensity)
{

    double Hue1 = 0, Saturation1 = 0, Intensity1 = 0;
    long Max, Min;

    Intensity1 = (Red + Green + Blue) / 3.0;

    double rn = Red / (Red + Green + Blue);
    double gn = Green / (Red + Green + Blue);
    double bn = Blue / (Red + Green + Blue);

    Hue1 = acos((0.5 * ((rn - gn) + (rn - bn))) / (sqrt((rn - gn) + (rn - bn) * (gn - bn))));
    if (Blue > Green)
    {
        Hue1 = 2 * M_PI - Hue1;
    }

    Saturation1 = 1 - 3 * min(rn, min(gn,bn));

    *pHue = Hue1;
    *pSaturation = Saturation1;
...
void HSItoRGB(double Hue, double Saturation, double Intensity, unsigned char *pRed, unsigned char *pGreen, unsigned char *pBlue)
{
    double Max = 0, Min = 0;
    unsigned char R, G, B;
    ...
    if(Hue < 2.0 * M_PI / 3.0)
    {
        double y = Intensity * (1.0 + (Saturation * cos(Hue)) / (cos(M_PI / 3.0 - Hue)));
        double z = 3.0 * Intensity - (x + y);
        *pBlue = x;
        *pRed = y;
        *pGreen = z;
    }
    ...
    {
        double y = Intensity * (1.0 + (Saturation * cos(Hue - 2 * M_PI / 3)) / (cos(M_PI / 3 - (Hue - 2 * M_PI / 3))));\
        double z = 3.0 * Intensity - (x + y);
        *pRed = x;
        *pGreen = y;
        *pBlue = z;
    ...
    {
        double y = Intensity * (1.0 + (Saturation * cos(Hue - 4 * M_PI / 3)) / (cos(M_PI / 3 - (Hue - 4 * M_PI / 3))));
        double z = 3.0 * Intensity - (x + y);
        *pRed = z;
        *pGreen  = x;
        *pBlue = y;
...
void EmbossImageRGB(unsigned char (*Red)[MAXS], unsigned char (*Green)[MAXS], unsigned char (*Blue)[MAXS], unsigned char (*Red2)[MAXS], unsigned char (*Green2)[MAXS], unsigned char (*Blue2)[MAXS], long Width, long Height, long Alpha)
{
    long i, j, row, col;
    unsigned char R = 0, G = 0, B = 0;
    double Hue = 0.0, Saturation = 0.0, Lightness = 0.0;
    ...
    for (j = 0; j < Height; j++) {
        for (i = 0; i < Width; i++) {
            ...
            R = Red[j + Alpha][i + Alpha] - Red[j][i];
            G = Green[j + Alpha][i + Alpha] - Green[j][i];
            B = Blue[j + Alpha][i + Alpha] - Blue[j][i];
...
void EmbossImageHSL(unsigned char (*Red)[MAXS], unsigned char (*Green)[MAXS], unsigned char (*Blue)[MAXS], unsigned char (*Red2)[MAXS], unsigned char (*Green2)[MAXS], unsigned char (*Blue2)[MAXS], long Width, long Height, long Alpha)
{
    long i, j, row, col;
    unsigned char R = 0, G = 0, B = 0, sum = 0;
    ...
    for(j = 0; j < Height; j++) {
        for(i = 0; i < Width; i++) {
            ...
            //Change RGB to HSI
            RGBtoHSI(Red[j][i], Green[j][i], Blue[j][i], &Hue, &Saturation, &Intensity);
    ...
    printf("%f %f", minIntensity, maxIntensity);
    ...
    for(j = 0; j < Height; j++) {
        for(i = 0; i < Width; i++) {
            ...
            //Contrast Reduction
            double newIntensity = 255 * ((Intensity - minIntensity) / (maxIntensity - minIntensity));
...
int main(void)
{
    ...
    ReadImage("osaka.ppm", Red, Green, Blue, &Width, &Height);
    EmbossImage(Red, Green, Blue, Red2, Green2, Blue2, Width, Height, 5.0);
    WriteImage("osaka_out.ppm", Red2, Green2, Blue2, Width, Height);

    //AlphablendImage (Red, Green, Blue,Red2, Green2, Blue2, alphablend_Red, alphablend_Green, alphablend_Blue, Width, Height, 0.25);
    //WriteImage("ghibli_out.ppm", alphablend_Red, alphablend_Green, alphablend_Blue, Width, Height);
